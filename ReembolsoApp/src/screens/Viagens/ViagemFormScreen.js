...
export default function ViagemFormScreen({ route, navigation }) {
    ...
    useEffect(() => {
        if (kmInicial && kmFinal) {
            const startKm = parseFloat(String(kmInicial).replace(',', '.'));
            const endKm = parseFloat(String(kmFinal).replace(',', '.'));
            if (!isNaN(startKm) && !isNaN(endKm) && endKm > startKm) {
                const dist = endKm - startKm;
                setDistancia(String(dist.toFixed(2)).replace('.', ',')); 
            } else {
                setDistancia('');
            }
        }
    ...
    const handleSave = async (isDraft = false) => {
        ...
        const distanciaParaApi = distancia ? String(distancia).replace(',', '.') : null;
        const kmInicialParaApi = kmInicial ? String(kmInicial).replace(',', '.') : null;
        const kmFinalParaApi = kmFinal ? String(kmFinal).replace(',', '.') : null;

        if (!isDraft && (!distanciaParaApi || parseFloat(distanciaParaApi) <= 0)) {
        ...
        const bodyData = {
            ...
            km_inicial: kmInicialParaApi,
            descricao: descricao,
            isDraft: isDraft,
            km_final: kmFinalParaApi,
            distancia_percorrida: distanciaParaApi,
                    ...
                    >
                        ...
                        <View style={{paddingHorizontal: 20}}>
                            ...
                            <View style={styles.inputWrapper}>
                                ...
                                <TextInput style={styles.input} value={localSaida} onChangeText={setLocalSaida} placeholder="Local de Saída" placeholderTextColor="#ccc" />
                            </View>
                            
                            <View style={styles.inputWrapper}>
                                <Icon name="location-outline" size={22} color="#fff" style={styles.icon} />
                                <TextInput style={styles.input} value={localChegada} onChangeText={setLocalChegada} placeholder="Local de Chegada" placeholderTextColor="#ccc" />
                            </View>
